...
if ($method === 'GET') {
  ...
  $stmt = db()->prepare('SELECT id,email,full_name,role,color_hex FROM users WHERE full_name LIKE ? OR email LIKE ? ORDER BY full_name LIMIT 100');
  $like = '%' . $q . '%';
...
if ($method === 'POST') {
  ...
  $stmt = $pdo->prepare('INSERT INTO users (email,password_hash,full_name,role,color_hex) VALUES (?,?,?,?,?)');
  try {
    $stmt->execute([$email, password_hash($password, PASSWORD_ARGON2ID), $fullName, $role, $colorHex]);
  } catch (Throwable $e) {
    json_error('Email already exists', 409);
  }
  $id = (int)$pdo->lastInsertId();
  $out = $pdo->prepare('SELECT id,email,full_name,role,color_hex FROM users WHERE id = ?');
  $out->execute([$id]);
...
if ($method === 'PATCH') {
  ...
  $in = body_json();
  $allowed = ['full_name', 'role', 'color_hex', 'password'];
  $patch = [];
